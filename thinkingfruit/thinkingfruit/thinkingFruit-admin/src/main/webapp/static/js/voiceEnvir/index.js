...
var voiceEnvir_index_ops={
		...
		eventBind:function(){
			...
			$(".btn-add").click(function(){
				window.location.href = WEB_ROOT + '/voiceEnvir/set';
			});
		},
				...
				table = layui.table;
				   ...
				   var tableIns = table.render({
				       ...
				       , cols: [[                          //标题栏
				           ...
				           , {field: 'name', title: '图片名称', align: 'center',width: 250}
				           , {field: 'imagePath', title: '封面图片', align: 'center',width: 150}
				           , {field: 'intro', title: '描述',align: 'center'}
				           , {field: 'createTime', title: '创建时间',templet:'#date_formate',align: 'center'}
				           , {title: '操作', width: 250,height: 40, align: 'center', templet: '#barOption'} //这里的toolbar值是模板元素的选择器
				       ]]
				       , id: 'dataCheck'
				       , url: WEB_ROOT + "/voiceEnvir/pagination"
				       , method: 'get'
				       , page: true
				       , limit: 10 //默认采用30
				       , limits :[10]
					...
					$(".btn-serach").on('click',function(){						
						...
						console.log(name+"dddddddddddddddddddddddddd");
						name = "任意设";
						tableIns.reload({
					...
					table.on('tool(table-data)', function(obj) {
						...
						if(obj.event == 'detail'){
							//修改新闻信息
							window.location.href = WEB_ROOT + '/voiceEnvir/set?id='+id;
						}else{
							//ajax删除
							var msg = "确认删除";
							var callback = {
								'ok' : function() {
									$.ajax({
										type : 'post',
										url : WEB_ROOT + "/voiceEnvir/" + data.id,
										data : {
											_method : 'delete',
											id : id
										},
