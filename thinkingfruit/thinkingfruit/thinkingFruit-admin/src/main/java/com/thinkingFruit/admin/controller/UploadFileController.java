 ...
 * @author OldHuang
 * 
 * @Package com.ysdevelop.cms.controller.manage
 * 
 * @Description: 用户上传文件controller,包括两种情况,上传img,video
 * 
 * @date 2017年11月2日
 * 
 * @version
...
public class UploadFileController {
	...
	@RequestMapping(value = "/video", method = RequestMethod.POST, produces = "text/json;charset=UTF-8")
	@ResponseBody
	public String uploadVideo(MultipartHttpServletRequest requestFile, HttpServletRequest request) {
		// 上传input name 为uploadImage
		MultipartFile file = requestFile.getFile("uploadVideo");
		
		Integer fileType = Integer.parseInt(requestFile.getParameter("videoType"));
		String modulePath = Constant.VIDEO_PATH.get(fileType);
		if (file == null || file.isEmpty()) {
			logger.info("上传的文件为空");
			//上传的文件为空
			return JSONHelper.bean2json(Result.error(CodeMsg.FILE_IS_EMPTY));
		}
		// 读取配置文上传件的路径
		String realPath = null;
		if (PropertiesConfigUtil.getProperty("upload.properties", "upload.video.path") != null) {
			realPath = PropertiesConfigUtil.getProperty("upload.properties", "upload.video.path").toString();
		}
		if (realPath == null) {
			//获取配置文件失败
			return JSONHelper.bean2json(Result.error(CodeMsg.CONFIGURATION_FAILURE));
		}

		String videoPath = null;
		videoPath = UploadUtil.doUpload(FileType.VIDEO, realPath, modulePath, file);
		...
		Map<String, Object> msgMap = new HashMap<>();
		msgMap.put("videoPath", videoPath);
		msgMap.put("videoFileName", file.getOriginalFilename());
